module.exports = function(appData) {
  ...
  appData.tasks.sprites = {
    run: function(taskParams) {
      ...
      if (!spriteData) {
        spriteData = gulp.src(appData.app.taskUtils.sanitizeGlob(taskParams.itemInfo.imgSource))
      ...
      var promise = appData.app.taskUtils.streamToPromise(merge(imgStream, cssStream));
      promise.then(() => {
        injector.run('reload');
        if (!injector.isCanceled) {
