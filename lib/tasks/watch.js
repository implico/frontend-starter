...
module.exports = function(appData) {
  ...
  appData.tasks.watch = {
    run: function(taskParams) {
      ...
      if (!injector.isCanceled) {
        gulp.watch(fontsGlob, _this.getTaskFn('fonts', () => {
      ...
      if (!injector.isCanceled) {
        config.sprites.items.forEach(function(itemInfo) {
          gulp.watch(itemInfo.src, _this.getTaskFn('sprites', () => {
      ...
      if (!injector.isCanceled) {
        gulp.watch(viewsGlob, _this.getTaskFn('views', () => {
      ...
      config.customDirs.items.forEach(function(dirInfo) {
        ...
        if (!injectorDir.isCanceled) {
          gulp.watch(dirInfo.src, _this.getTaskFn('custom-dirs' + (typeof dirInfo.name !== 'undefined' ? (' [' + dirInfo.name + ']') : '') , () => {
