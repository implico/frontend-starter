module.exports = function(appData) {
  ...
  appData.tasks.js = {
    ...
    run: function(taskData) {
      ...
      if (babelPresets.indexOf('es2015') >= 0) {
        delete babelPresets[babelPresets.indexOf('es2015')];
        injector.taskConfig.babel.options.presets = babelPresets.filter(function(v) { return v; });
      ...
      if (taskData.isApp) {
        ...
        if (!injector.isCanceled) {
          stream = appData.tasks.lint.performLint(stream, null, injectorData, injector);
        }
      }
      ...
      if ((taskData.isApp && babelConfig.app) || (!taskData.isApp && babelConfig.vendor)) {
        stream = injector.run('babel', stream, injectorData);
        if (!injector.isCanceled) {
