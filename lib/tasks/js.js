    ...
    run: function(taskData) {
      ...
      if (!injector.cancel) {
        if (taskData.isApp) {
          ...
          stream = gulp.src(stream ? stream: mainBowerFiles(configJs.mainBowerFiles), { base: dirs.src.main })
      ...
      if (configJs.sourceMaps) {
        ...
        if (!injector.cancel) {
          stream = stream.pipe(sourcemaps.write({ includeContent: false, sourceRoot: configJs.sourceMapsRoot }))
        }
      }
      ...
      if (!injector.cancel) {
        if (taskData.isApp) {
          stream = stream.pipe(appData.app.browserSync.stream());
        }
      }
